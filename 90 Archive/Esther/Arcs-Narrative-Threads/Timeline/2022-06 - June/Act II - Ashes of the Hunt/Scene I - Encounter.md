---
ID: scn_7172d872
Class: Scene
Act: 1
When: 2022-06-09T8:30
Synopsis: A chance encounter.
Subplot:
  - Ashes of the Hunt
Character:
  - Halay
Place:
  - Lake Axewell
  - Galar
Status: Complete
Publish Stage: Zero
Revision: ""
Due:
Pending Edits: ""
Words: 0
Beats Update: No
sticker: lucide//crosshair
Duration: 4 Hours
---


```mermaid
flowchart TD
A(Houndour Encounter/Capture) --> B(Nurse Joy)
B --> |Aromatherapy| C(Calms Houndour)
B ---> |Discover Tracker| D(Contacts Rangers & Rader-Engineer)
B ~~~ |Villain POV| E(Aurelian talks to Rhys)
E --> F(Predicts they'd be reverse-tracked; rigs the ship)
```


#poke-pov: Houndour
- She's scared & Confused, awakening in an unfamiliar land—who knows where—with her last memories being snatched away from her family. Nevertheless, she brandishes her fangs; to show weakness is to invite offense, this is two-fold for a leader lest their uncertainty trickle down.


#seed-planted
- Aromatherapy:  Esther wants a Pokemon w/ Aromatherapy to help her calm down Houndour, it would also help w/ Pyrrha. 

#team-levels : 2022-06-09
-  Pyrrha Lv.12 $\to$ 14

#team-battles: Pyrrha
	- vs Wilford
	- vs Minccino
	- vs Houndour

#team-additions:
- Houndour Lv.15
