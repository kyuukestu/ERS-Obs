---
ID: scn_19d6f19d
Class: Scene
Act: 1
When: 2022-06-09T14:30
Synopsis: The search for and search of the derelict of J's ship.
Subplot:
  - Ashes of the Hunt
Character:
  - Halay
Place:
  - Lake Axewell
  - Galar
Status: Complete
Publish Stage: Zero
Revision: ""
Due:
Pending Edits: ""
Words: 0
Beats Update: No
sticker: lucide//crosshair
Duration: 4 Hours
---
Esther & Co. Search for and locate the derelict of Hunter J's ship. It's rusted and suffering from severe water-damage; it was evidently restored to *some* degree of functionality. Wynter speculates this reduced functionality led to this 'crash landing'.

```mermaid
flowchart TD
A(Search for Ship) --> B(Explore Ship) --> |Discover Ship Rigged| C(Daring Escape) --> |Wynter pursues leads| D(Aurelian uses Mirage J to throw off their pursuers)
```
